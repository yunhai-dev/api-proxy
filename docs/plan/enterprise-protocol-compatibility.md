...
- **Specific logic**:
  ...
  - Add atomic quota reservation/reconciliation, bounded cancellation-aware admission, and circuit-breaker behavior using the existing health records. Completed: Redis TPM reservation/reconciliation, bounded cancellation-aware queues, lease renewal, a 30-second closed/open/half-open circuit cooldown, and PostgreSQL transactions covering request logs, Key/user usage, and request statistics. A partial unique index on Key + request ID prevents duplicate billing; complete integration coverage remains pending.
