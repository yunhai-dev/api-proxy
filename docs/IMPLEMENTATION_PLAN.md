...
| 4 | 上游传输、配额与可靠性加固 | [x] | 传输边界、原子 TPM、队列与闭环/半开熔断已加固；上游已接受但使用量未知的失败会保守保留本请求 TPM 预留；已覆盖代理重试/回退共享同一预留并按实际使用量结算；请求日志、Key/用户额度与统计事实已在同一 PostgreSQL 事务内持久化，并以 Key + 请求 ID 去重计费；已覆盖 stream 完成/取消时的 TPM 结算与槽位释放，并以 PostgreSQL-mode fixture 覆盖重复 Key + 请求 ID 不重复记 Key、用户额度或统计；代理 fixture 覆盖 open circuit 在渠道槽位申请前排除路由以及上游失败记录熔断观测后释放槽位，已覆盖熔断状态转换；本地 PostgreSQL/Redis schema 初始化与多实例验证已通过请求 ID 幂等、分布式信号量和 pub/sub；经受控 OpenAI stub 的本地生产代理调用成功并写入一条 200/2 入/3 出 token 的请求日志和统计记录，随后已清理种子数据；经延迟受控上游验证 channel concurrency 1 时两条代理请求由 Redis 租约串行执行（两次 350ms 响应总计 834ms），随后已清理种子数据 |
| 5 | 可观测性、契约测试与分批发布 | [x] | 已记录协议方向、成功重试/降级链、上游请求 ID、选中能力画像及桥接转换拒绝事实；已提供不含载荷、不会放行不兼容转换的桥接能力审计开关；管理总览按审计覆盖范围展示原生/桥接方向、失败/拒绝与时延，未审计记录明确标为未分类；已补充支持控制的语义保持/拒绝及上游路径/Retry-After 契约，覆盖代理级原生 OpenAI Chat/Responses/Claude 控制透传、成功 OpenAI Chat/Responses→Claude 与 Claude→OpenAI JSON 响应转换、Chat 双向及 Responses→Claude 流式响应转换及不兼容桥接请求不会触发主/回退上游；已提供需显式凭据的原生/桥接渠道烟测脚本，可选覆盖 Messages 与 Chat 的 SSE；用户决定不执行真实部署渠道 smoke，故该外部验证已豁免，未声称其通过 |
