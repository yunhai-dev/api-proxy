...
| 4 | 上游传输、配额与可靠性加固 | [ ] | 传输边界、原子 TPM、队列与闭环/半开熔断已加固；上游已接受但使用量未知的失败会保守保留本请求 TPM 预留；已覆盖代理重试/回退共享同一预留并按实际使用量结算；请求日志、Key/用户额度与统计事实已在同一 PostgreSQL 事务内持久化，并以 Key + 请求 ID 去重计费；已覆盖 stream 完成/取消时的 TPM 结算与槽位释放；已覆盖熔断状态转换；仍缺 durable accounting、队列/租约与熔断恢复等完整代理集成覆盖 |
| 5 | 可观测性、契约测试与分批发布 | [ ] | 已记录协议方向、成功重试/降级链、上游请求 ID、选中能力画像及桥接转换拒绝事实；已提供不含载荷、不会放行不兼容转换的桥接能力审计开关；管理总览按审计覆盖范围展示原生/桥接方向、失败/拒绝与时延，未审计记录明确标为未分类；已补充支持控制的语义保持/拒绝及上游路径/Retry-After 契约，覆盖代理级原生 OpenAI 控制透传、成功 OpenAI→Claude JSON 响应转换及不兼容桥接请求不会触发主/回退上游；已提供需显式凭据的原生/桥接渠道烟测脚本，可选覆盖 Messages 与 Chat 的 SSE；实际渠道执行待部署环境完成 |
