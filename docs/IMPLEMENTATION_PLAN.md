...
| 5 | 可观测性、契约测试与分批发布 | [ ] | 已记录协议方向、成功重试/降级链、上游请求 ID、选中能力画像及桥接转换拒绝事实；已提供不含载荷、不会放行不兼容转换的桥接能力审计开关；管理总览按审计覆盖范围展示原生/桥接方向、失败/拒绝与时延，未审计记录明确标为未分类；已补充支持控制的语义保持/拒绝及上游路径/Retry-After 契约，覆盖代理级原生 OpenAI Chat/Responses/Claude 控制透传、成功 OpenAI Chat/Responses→Claude 与 Claude→OpenAI JSON 响应转换、Chat 双向及 Responses→Claude 流式响应转换及不兼容桥接请求不会触发主/回退上游；已提供需显式凭据的原生/桥接渠道烟测脚本，可选覆盖 Messages 与 Chat 的 SSE；实际渠道执行待部署环境完成 |
