...
function UserKeysTable({ rows }: { rows: DetailKey[] }) {
  ...
  return (
    <section className="list-section">
      <h2>绑定 API Key</h2>
      <div className="list-toolbar"><Input tone="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="搜索 Key 名称 / 前缀" /><Select value={statusFilter} onChange={setStatusFilter} options={[{ value: "all", label: "全部状态" }, { value: "active", label: "活跃" }, { value: "disabled", label: "已停用" }]} /><span className="spacer" /><span className="mono dim">{filtered.length} 个 Key</span></div>
      <div className="table-wrap"><table className="table"><thead><tr>{sortHeader("name", "名称")}{sortHeader("prefix", "前缀")}{sortHeader("status", "状态")}{sortHeader("requests", "区间请求")}{sortHeader("tokens", "区间 Token")}{sortHeader("cost", "区间消费")}</tr></thead><tbody>{pageRows.length === 0 && <tr><td colSpan={6} className="empty">暂无匹配 Key <span className="mono dim">// no rows</span></td></tr>}{pageRows.map(k => <tr key={k.id}><td>{k.name}</td><td className="mono">{k.prefix}</td><td>{k.status}</td><td className="mono">{k.periodStats.requests}</td><td className="mono">{k.periodStats.tokens.toLocaleString()}</td><td className="mono">${k.periodStats.cost.toFixed(4)}</td></tr>)}</tbody></table></div>
...
function UserModelsTable({ rows }: { rows: ModelStat[] }) {
  ...
  return (
    <section className="list-section">
      <h2>模型统计</h2>
      <div className="list-toolbar"><Input tone="search" value={query} onChange={e => setQuery(e.target.value)} placeholder="搜索模型" /><span className="spacer" /><span className="mono dim">{filtered.length} 个模型</span></div>
      <div className="table-wrap"><table className="table"><thead><tr>{sortHeader("model", "模型")}{sortHeader("requests", "请求")}{sortHeader("tokens", "Token")}{sortHeader("cost", "消费")}</tr></thead><tbody>{pageRows.length === 0 && <tr><td colSpan={4} className="empty">暂无匹配模型数据 <span className="mono dim">// no rows</span></td></tr>}{pageRows.map(m => <tr key={m.model}><td className="mono">{m.model}</td><td className="mono">{m.requests}</td><td className="mono">{m.tokens.toLocaleString()}</td><td className="mono">${m.cost.toFixed(4)}</td></tr>)}</tbody></table></div>
      <ListPagination page={safePage} pageSize={pageSize} total={filtered.length} onPageChange={setPage} onPageSizeChange={setPageSize} />
    </section>
  );
}
