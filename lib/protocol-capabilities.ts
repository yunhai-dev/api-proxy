...
export function requiredCapabilities(input: {
  ...
  if (input.sourceType === "openai" && input.openAiEndpoint === "responses" && Array.isArray(input.body.input) && input.body.input.some(item => isRecord(item) && (item.type === "function_call" || item.type === "function_call_output"))) required.add("tool_replay");
...
export function routeSupportsCapabilities(input: {
  ...
  if (input.sourceType === input.targetType) return true;
